 ...
 * The {@link #calculateTangents(VertexNormal, VertexNormal, VertexNormal, Vector2f[])}
...
{
    // global-singleton stuff
    private static Vector3f vertex;
    ...
    public static Model loadObjModel(String filename) {
        ...
        fileReader = Main.class.getResourceAsStream("objects/" + filename);
    ...
    public static Model loadObject(String objFileName) {
        ...
        InputStream objFile = Main.class.getResourceAsStream("objects/" + objFileName);
        assert objFile != null;
        ...
        try {
            ...
            VertexNormal[] vertArray = new VertexNormal[vertices.size()];
            texArray = new Vector2f[textures.size()];
            normalArray = new Vector3f[normals.size()];

            for(int i = 0; i < vertices.size(); i++)
                vertArray[i] = vertices.removeFirst();
            ...
            while (line != null && line.startsWith("f ")) {
                ...
                String[] vertex2 = currentLine[2].split("/");
                String[] vertex3 = currentLine[3].split("/");
                VertexNormal v0 = processVertex(vertex1, vertArray, indices);
                VertexNormal v1 = processVertex(vertex2, vertArray, indices);
                VertexNormal v2 = processVertex(vertex3, vertArray, indices);
                calculateTangents(v0, v1, v2, texArray);
                line = reader.readLine();
            }
            reader.close();
        } catch (IOException e) {
        ...
        convertDataToArrays(vertices, texArray, normalArray, verticesArray, texturesArray, normalsArray, tangentsArray);
        int[] indicesArray = convertIndicesLinkListToArray(indices);

        return ModelLoader.loadToVaoInt(verticesArray, texturesArray, normalsArray, indicesArray, tangentsArray);
    }

    private static void calculateTangents(VertexNormal v0, VertexNormal v1, VertexNormal v2, Vector2f[] textures) {
    ...
    private static VertexNormal processVertex(String[] vertex, VertexNormal[] vertices, LinkList<Integer> indices) {
        int index = Integer.parseInt(vertex[0]) - 1;
        VertexNormal currentVertex = vertices[index];
    ...
    private static int[] convertIndicesLinkListToArray(LinkList<Integer> indices) {
    ...
    private static void convertDataToArrays(LinkList<VertexNormal> vertices, Vector2f[] textures, Vector3f[] normals,
                                            float[] verticesArray, float[] texturesArray, float[] normalsArray, float[] tangentsArray) {
        for (int i = 0; i < vertices.size(); i++) {
            VertexNormal currentVertex = vertices.removeFirst();
            Vector3f position = currentVertex.getPosition();
            Vector2f textureCoords = textures[currentVertex.getTextureIndex()];
    ...
    private static VertexNormal dealWithAlreadyProcessedVertex(VertexNormal previousVertex, int newTextureIndex, int newNormalIndex,
                                                               LinkList<Integer> indices, LinkList<VertexNormal> vertices) {
...
//    }
    ...
    private static void removeUnusedVertices(LinkList<VertexNormal> vertices) {
        for (VertexNormal vertex : vertices) {
            vertex.averageTangents();
            if (!vertex.isSet()) {
                vertex.setTextureIndex(0);
